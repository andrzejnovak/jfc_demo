...
def fix_longtable_short(lines):
    ...
    Self-cleaning: after conversion this strips longtable-only remnants
    that pandoc's longtable boilerplate leaves behind — `\endfirsthead`,
    the duplicated firsthead header rows, stray `\endhead/\endfoot/
    \endlastfoot` — relocates any in-body `\caption` to just after
    `\centering`, and ensures the tabular has a closing `\bottomrule`.

    If any longtable-only remnant survives the cleanup (defensive: should
    not happen, but a malformed longtable could slip through), it prints a
    LOUD `WARNING: longtable remnant left ...` to stderr so the typesetter
    knows to clean it.  It NEVER raises — postprocessing must not abort the
    build.
    """
    count = 0
    text = ''.join(lines)

    # Longtable-only commands that must not survive into a tabular float.
    remnant_re = re.compile(
        r'\\endfirsthead|\\endhead|\\endfoot|\\endlastfoot')

    # Find each longtable block.  The column spec may contain one level of
    # nested braces (pandoc emits specs like {@{}lll@{}} or
    # {@{}>{\raggedright\arraybackslash}p{...}@{}}), so the col-spec group
    # matches a brace group that allows a single level of nesting.  A flat
    # `\{[^}]*\}` would truncate `{@{}lll@{}}` to `{@{}`, producing an
    # unbalanced \begin{tabular}{@{} that fails to compile.
    pattern = re.compile(
        r'(\\begin\{longtable\}(?:\[[^\]]*\])?)'
        r'(\{(?:[^{}]|\{[^{}]*\})*\})'
        r'(.*?)(\\end\{longtable\})',
    ...
    def convert_if_short(match):
        ...
        data_lines = [
            ...
            and '\\endfirsthead' not in l
            and '\\caption' not in l
        ]
        if len(data_lines) >= 15:
            return match.group(0)  # keep as longtable

        # --- Extract caption (in-body) before stripping anything else ---
        # Longtable captions look like:  \caption{...}\label{...}\\
        # We pull it out so we can relocate it to just after \centering.
        new_body = body
        caption = ''
        ...
        if caption_match:
            ...
            new_body = new_body.replace(caption_match.group(0), '', 1)

        # --- Strip the longtable firsthead/head/foot boilerplate ---
        # Pandoc emits:
        #     <header rows> \\ \endfirsthead
        #     <header rows> \\ \endhead
        #     ... body ...
        #     \endlastfoot  (or \endfoot)
        # The firsthead block is a duplicate of the endhead header block.
        # Remove the entire firsthead block (everything up to and including
        # \endfirsthead) since the endhead block carries the canonical
        # header for a single-page tabular.
        new_body = re.sub(
            r'.*?\\endfirsthead\s*\n?', '', new_body, count=1, flags=re.DOTALL)
        # Remove the trailing \endhead marker but keep the header rows that
        # precede it (those become the tabular's header).
        new_body = re.sub(r'\\endhead\s*\n?', '', new_body)
        # Remove foot markers (and any foot rows pandoc placed before them
        # are typically empty for short tables).
        new_body = re.sub(r'\\endlastfoot\s*\n?', '', new_body)
        new_body = re.sub(r'\\endfoot\s*\n?', '', new_body)

        # --- Ensure a closing \bottomrule ---
        # booktabs longtables end the data with \bottomrule; if the strip
        # above removed it (or it was in a foot block), re-add one.
        stripped_body = new_body.strip()
        if '\\bottomrule' not in stripped_body:
            stripped_body += '\n\\bottomrule'

        # --- Build table float with caption right after \centering ---
        ...
        # --- Defensive remnant check: warn loudly, never raise ---
        leftover = remnant_re.search(result)
        if leftover:
            # Try to name the table by its caption/label for the message.
            label_m = re.search(r'\\label\{([^}]*)\}', result)
            name = label_m.group(1) if label_m else (
                caption.strip()[:60] if caption else 'unnamed table')
            sys.stderr.write(
                f"WARNING: longtable remnant ({leftover.group(0)}) left in "
                f"<{name}>, typesetter must clean\n")
        return result
